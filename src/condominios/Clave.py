...
def Clave_do_Sol(pdf_filename):
    ...
    if not os.path.exists(pdf_path):
        ...
        # BLOCO A
        padrao_bloco_A = r"CS A (\d{4}(?:-\d{2,4})?)\s+.*?Apto:\s*(\d+)(.*?)\n(?=CS A|\Z)"
        resultados_bloco_A = re.findall(padrao_bloco_A, texto_pdf, flags=re.DOTALL)

        # BLOCO B
        padrao_bloco_B = r"CS B (\d{4}(?:-\d{2,4})?)\s+.*?Apto:\s*(\d+)(.*?)\n(?=CS B|\Z)"
        resultado_bloco_B = re.findall(padrao_bloco_B, texto_pdf, flags=re.DOTALL)

        # BLOCO C
        padrao_bloco_C = r"CS C (\d{4}(?:-\d{2,4})?)\s+.*?Apto:\s*(\d+)(.*?)\n(?=CS C|\Z)"
        resultado_bloco_C = re.findall(padrao_bloco_C, texto_pdf, flags=re.DOTALL)

        # BLOCO D
        padrao_bloco_D = r"CS D (\d{4}(?:-\d{2,4})?)\s+.*?Apto:\s*(\d+)(.*?)\n(?=CS D|\Z)"
        resultado_bloco_D = re.findall(padrao_bloco_D, texto_pdf, flags=re.DOTALL)
        ...
        for resultado in resultados_bloco_A:
            ...
            # Remover os dois primeiros dígitos do protocolo, se começar com "00"
            if protocolo.startswith("00"):
                protocolo = protocolo[2:]

            # Remover os dois primeiros dígitos das salas, se começar com "0"
            if numero_sala.startswith("0"):
        ...
        # Lançando o Bloco B
        for resultado in resultado_bloco_B:
            numero_sala, protocolo, conteudo_sala = resultado
            ...
            valor_total = valores[-1] if valores else "Valor nao encontrado"
        ...
        # Lançando Bloco C
        for resultado in resultado_bloco_C:
            numero_sala, protocolo, conteudo_sala = resultado
        ...
        # Lançando Bloco D
        for resultado in resultado_bloco_D:
            numero_sala, protocolo, conteudo_sala = resultado
