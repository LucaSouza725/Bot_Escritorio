...
def New_World(pdf_filename):
    ...
    if not os.path.exists(pdf_path):
        ...
        padrao_salas = r"NW (\d{4}(?:-\d{2,4})?)\s.*?Apto:\s*(\d+)(.*?)(?=\nNW|\Z)"
        ...
        padrao_garagens = r"NW G\s+(\d+).*?Apto:\s*(\d+)(.*?)(?=NW G|NW LOJA 1|$)"
